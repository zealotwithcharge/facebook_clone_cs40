...
def compile_headers(line):
    ...
    if number == 0:
        ...
        return f'<h{number}>{line[number:]}</h{number}>'
...
def compile_italic_star(line):
    ...
    >>> compile_italic_star('\*\*')
    '**'
    >>> compile_italic_star('\*this is not italics\*')
    '*this is not italics*'
    >>> compile_italic_star('\*ao\*eu\*')
    '*ao*eu*'
    '''
    # flag = True
    # holder = ''
    ...
    #     if line[i] != '*':
    #         if not(i != len(line) -1 and line[i+1] == '*' and line[i] == '\\'):
    #             holder += line[i]
    ...
    for i,word in enumerate(word_list):
        if i!=0 and i!=len(word_list)-1:
            if  word != "" and word[-1] != '\\' and word_list[i-1].find(end_tag) == -1:
            ...
            elif word_list[i-1].find(end_tag) != -1:
                word_list[i] = word
            else:
...
def compile_italic_underscore(line):
    ...
    for i,word in enumerate(word_list):
        if i!=0 and i!=len(word_list)-1:
            ...
            if (word_list[i-1]=="" or word_list[i-1][-1] == " " or word_list[i-1][-1] in string.punctuation) and (word_list[i+1] == "" or word_list[i+1][0] == " " or word_list[i+1][0] in string.punctuation) and word != "" and word[-1] != '\\' and word_list[i-1].find(end_tag) == -1:
                word_list[i] = tag+word+end_tag
...
def compile_strikethrough(line):
    ...
    '<del>This is strikethrough!</del> This is not strikethrough.'
...
def compile_code_inline(line):
    ...
    for i in range(len(line)):
        if line[i] == '<' and not flag:
            ...
            if line[i+1:].find('`') != -1 and not (i < len(line)-2 and line[i+1] == '`' and line[i+2] == '`'):
                if i != 0 and line[i-1]=='`':
                    holder += '`'
                else:
                    holder += '<code>'
...
def compile_links(line):
    ...
    start_index_braket = line.find('[')
    end_index_braket = line.find(']')
    start_index_paran = line.find('(')
    end_index_paran = line.find(')')
    text = line[start_index_braket+1:end_index_braket]
    link = line[start_index_paran+1:end_index_paran]
    if start_index_paran - end_index_braket == 1 and start_index_braket != -1 and end_index_braket !=-1 and start_index_paran !=-1 and end_index_paran != -1:
        holder = line[:start_index_braket] + f'<a href="{link}">{text}</a>'+line[end_index_paran+1:]
    else:
        holder = line
    return holder
...
def compile_images(line):
    ...
    text = line[start_index_braket+1:end_index_braket]
    link = line[start_index_paran+1:end_index_paran]
    if exclam != -1 and start_index_braket - exclam== 1 and start_index_paran - end_index_braket == 1 and start_index_braket != -1 and end_index_braket !=-1 and start_index_paran !=-1 and end_index_paran != -1:
        holder = line[:exclam] + f'<img src="{link}" alt="{text}" />'+line[end_index_paran+1:]
    else:
        holder = line
    return holder
...
def compile_lines(text):
    ...
    for line in lines:
        ...
        if line.find(f'{place}.') == 0 and not_in_pre:
            pot_list = True
            ol.append(line)
            place += 1
            continue
        elif pot_list and not_in_pre:
            listline += '<ol>'
            for li in ol:
                listline += ('<li>'+li[2:].strip() +'</li>')
            listline += '</ol>'
            new_lines.append(listline)
            place = 1
            pot_list = False
            ol = []
            listline = ''
        ...
        if line=='':
            ...
            if line[0] != '#' and not in_paragraph:
                if len(lines) > 1 and lines[1].find('```')==-1:
                    in_paragraph = True
                    line = '<p>\n'+line
            if line.find('```') != -1:
                if not_in_pre:
...
def convert_file(input_file, add_css):
    ...
    new_html = html.split('<pre>')
    new_html2 = []
    new_html3 = []
    for line in new_html:
        new_html2.append(line.split('</pre>'))

    for line in new_html2:
        if len(line) == 1:
            new_html3.append(minify(line[0]))
        else:
            new_html3.append('<pre>')
            new_html3.append(line[0])
            new_html3.append('</pre>')
            new_html3.append(minify(line[1]))
